...
function DocuListS1() {
    ...
    const filteredBoardData = Array.isArray(boardList) ? boardList.filter(item => item.refCategoryNo === 7) : [];
        ...
        <div className={TableCSS.container}>
            <div className={TableCSS.section}>
                <figcaption className={TableCSS.docuList}>
                    ...
                    <ul>
                        <NavLink to="/board/notieM"><li>공지사항</li></NavLink><strong className={TableCSS.strong}>{filteredBoardData.length}</strong>
                        <NavLink to="/board/notieM"><MdKeyboardDoubleArrowRight/></NavLink>
                    </ul>
                </figcaption>
