...
function DocuListS3() {
    ...
    const filteredBoardData = Array.isArray(boardList) ? boardList.filter(item => item.refCategoryNo === 9) : [];
        ...
        <div className={TableCSS.container}>
            <div className={TableCSS.section}>
                <figcaption className={TableCSS.docuList}>
                    ...
                    <ul>
                        <NavLink to="/board/freeM"><li>자유게시판</li></NavLink><strong className={TableCSS.strong}>{filteredBoardData.length}</strong>
                        <NavLink to="/board/freeM"><MdKeyboardDoubleArrowRight/></NavLink>
                    </ul>
                </figcaption>
                    ...
                    <table className={TableCSS.docuContent}>
                        ...
                        {filteredBoardData.slice(0, 2).map((item, index) => (
                            <tr key={index} className={TableCSS.content} >
                                ...
                                <td>{item.regist}</td>
                                <td>{item.anonymity === 'N' ? `${item.storeName}` : '익명'}</td>
                            </tr>
                        ))}
                    </table>
                </div>
