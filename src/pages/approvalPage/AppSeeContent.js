...
import {callGetSeeWaitAPI} from "../../apis/ApprovalAPICalls";
import {Link, useNavigate} from "react-router-dom";

function AppSeeContent() {

    const dispatch = useDispatch();
    const result = useSelector(state => state.approvalSeeWaitReducer);
            ...
            <div className={style.waitingDoc}>
                <table className={style.table1}>
                    ...
                    {sendList && sendList.map(list => (
                        <tr>
                            <td className={style.td0}><input type="checkbox" name={AppWait.checkOne} className={AppWait.checkOne}/>{list.document.applicationDate.toString().substring(0,10)}</td>
                            <td className={style.td1} onClick={e => onClickHandler(e)}>{list.document.documentTitle}</td>
                            <input type="hidden" value={list.document.documentCode}/>
                            <td className={style.td0}>{list.document.documentType}</td>
                            <td className={style.td0}>{list.status}</td>
                            <td className={style.td0}>{list.document.employee.name}</td>
                        </tr>
                    ))}
                    </tbody>
                </table>
